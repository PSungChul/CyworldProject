...
<meta charset="UTF-8">
<title>사진첩 수정</title>
<link rel="stylesheet" href="/cyworld_mysql/resources/css/gallery_modify.css">
									...
									<td>
										...
										<c:if test="${ vo.galleryFileName ne 'no_file' }">
											...
											<c:if test="${ vo.galleryFileExtension eq 'image' }">
												<img src="/cyworld_mysql/resources/upload/${ vo.galleryFileName }" width="100"/>
											</c:if>
											<!-- 확장자가 video일 경우 -->
											<c:if test="${ vo.galleryFileExtension eq 'video' }">
												<!-- video태그 autoplay : 자동 재생 / controls loop : 반복 재생 / muted : 음소거 -->
												<video autoplay controls loop muted src="/cyworld_mysql/resources/upload/${ vo.galleryFileName }" width="100"/>
											</c:if>
										</c:if>
